    ...
    <div className="space-y-16 animate-in fade-in duration-1000">
      <div className="space-y-8">
        <div className="flex flex-col 2xl:flex-row justify-between items-start 2xl:items-end gap-12">
          <div className="max-w-3xl">
            ...
            <p className="text-gray-400 font-bold text-2xl leading-relaxed max-w-xl">Intelligent spatial-trade terminal with localized AI analysis tools.</p>
          </div>
          ...
          <div className="flex flex-wrap gap-6 w-full 2xl:w-auto items-center">
             <div className="flex bg-white rounded-[3rem] shadow-2xl border border-gray-100 px-10 py-6 flex-grow min-w-[320px] focus-within:ring-8 focus-within:ring-emerald-50 transition-all group">
                <i className="fas fa-search text-emerald-500 self-center mr-8 text-2xl transition-transform"></i>
                <input 
                  type="text" 
                  placeholder="Asset search (e.g. Tractor, Land)..."
             ...
             <div className="flex bg-white p-2 rounded-[2rem] border border-gray-100 shadow-xl">
                <button onClick={() => setViewMode('grid')} className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all ${viewMode === 'grid' ? 'bg-emerald-600 text-white shadow-lg' : 'text-gray-400 hover:bg-gray-50'}`}><i className="fas fa-th-large"></i></button>
                <button onClick={() => setViewMode('list')} className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all ${viewMode === 'list' ? 'bg-emerald-600 text-white shadow-lg' : 'text-gray-400 hover:bg-gray-50'}`}><i className="fas fa-list"></i></button>
             </div>
        ...
        <div className="flex overflow-x-auto space-x-6 pb-8 no-scrollbar">
          {categories.map(cat => (
            <button 
              ...
              className={`px-14 py-6 rounded-[2.5rem] whitespace-nowrap font-black text-[11px] uppercase tracking-[0.4em] transition-all shadow-xl ${
      ...
      {compareList.length > 0 && (
        <div className="bg-gray-900 rounded-[4rem] p-12 text-white shadow-2xl relative overflow-hidden animate-in slide-in-from-bottom-10">
           ...
           {compareResult && (
              <div className="mt-12 p-10 bg-white/5 rounded-[3.5rem] border border-white/10 animate-in fade-in zoom-in duration-1000 whitespace-pre-wrap leading-relaxed text-gray-300">
                {compareResult}
              </div>
           )}
        </div>
...
const ProductCard: React.FC<{ 
  ...
  return (
    <div className={`bg-white rounded-[3.5rem] overflow-hidden shadow-xl border border-gray-100 hover:shadow-2xl transition-all group ${viewMode === 'list' ? 'flex items-center gap-8 p-6' : 'flex flex-col'}`}>
      <div className={`relative overflow-hidden shrink-0 ${viewMode === 'list' ? 'w-48 h-48 rounded-[2rem]' : 'aspect-square'}`}>
        <img src={product.imageUrl} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-[2000ms]" />
        {isPromoted && <span className="absolute top-4 left-4 bg-blue-600 text-white text-[8px] font-black px-4 py-1.5 rounded-full uppercase tracking-widest">Promoted</span>}
      </div>
      <div className={`flex-grow ${viewMode === 'list' ? 'p-0' : 'p-10'} space-y-6 relative`}>
        <div>
           <h3 className="font-black text-gray-900 text-2xl line-clamp-1 mb-1">{product.name}</h3>
           <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{product.category}</p>
        </div>
        <p className="font-black text-emerald-700 text-3xl">ZMW {product.price.toLocaleString()}</p>
        
        <div className="flex gap-4">
           <button onClick={() => setShowContact(true)} className="flex-grow py-4 bg-emerald-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-emerald-700 transition">Acquire</button>
           <button onClick={onWishlist} className={`w-12 h-12 rounded-2xl flex items-center justify-center transition ${isWishlisted ? 'bg-red-500 text-white shadow-lg' : 'bg-gray-50 text-gray-400 hover:text-red-500'}`}>
              <i className="fas fa-heart"></i>
           </button>
           <button onClick={onCompare} className="w-12 h-12 bg-gray-50 rounded-2xl flex items-center justify-center text-gray-400 hover:text-emerald-600 transition">
              <i className="fas fa-plus"></i>
           </button>
        </div>

        {showContact && (
          <div className="absolute inset-0 bg-gray-950/95 backdrop-blur-xl z-20 flex flex-col items-center justify-center p-8 rounded-[3.5rem] text-white animate-in fade-in duration-300">
